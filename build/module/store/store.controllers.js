...
let getStoreList = exports.getStoreList = (() => {
  var _ref = _asyncToGenerator(function* (req, res) {
    ...
    try {
      const list = yield _store2.default.list({ skip, limit });
      const total = yield _store2.default.count({ isRemoved: false });
...
let getStoreHistory = exports.getStoreHistory = (() => {
  var _ref4 = _asyncToGenerator(function* (req, res) {
    ...
    try {
      ...
      const histories = yield _storeHistory2.default.list({ skip, limit, store: store._id });
      let totalQuantity = 0;
      let totalPrice = 0;
      ...
      const total = yield _storeHistory2.default.count({ store: store._id, isRemoved: false });
    ...
    try {
      ...
      const products = yield Promise.all(productList.map((() => {
        var _ref7 = _asyncToGenerator(function* ({ importPrice, exportPrice, quantity = 0 }) {
          ...
          const product = yield _product2.default.findOne({ store: storeId, importPrice, exportPrice });
          if (product) {
