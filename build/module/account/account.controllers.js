      ...
      case 'MANAGER':
        {
          ...
          return res.status(_httpStatus.default.OK).json({ ...accountDetail.toJSON(),
            ...account.toAuthJSON()
          });
        }
...
const createAccount = async (req, res) => {
  try {
    ...
    const account = await _account.default.create({ ...req.body
    });
    ...
    switch (role) {
      case 'CUSTOMER':
        accountDetail = await _customer.default.create({
          account: account._id,
          fullName
        });
        break;
    ...
    await account.save();
    return res.status(_httpStatus.default.CREATED).json({ ...account.toAuthJSON(),
      ...accountDetail.toJSON()
    });
  } catch (error) {
    console.log(error);
