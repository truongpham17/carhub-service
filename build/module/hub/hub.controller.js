...
const getHubById = async (req, res) => {
  try {
    ...
    const cars = await _car.default.find({
      currentHub: hub._id.toString()
    }).populate('carModel customer');
    return res.status(_httpStatus.default.OK).json({ ...hub.toJSON(),
      cars
    });
  } catch (error) {
    return res.status(_httpStatus.default.BAD_REQUEST).json(error.message);
...
const updateHub = async (req, res) => {
  try {
    ...
    const hub = await _hub.default.findById(id);
    Object.keys(req.body).forEach(key => {
      hub[key] = req.body[key];
    });
    await hub.save();
    return res.status(_httpStatus.default.OK).json(hub);
  } catch (error) {
    return res.status(_httpStatus.default.BAD_REQUEST).json(error.message);
  }
};
